...
def init_model(args, configs):
    ...
    if decoder_type != "none" and language_specific_layers:
    ...
    # carry values from checkpoint to configs
    configs["num_seen_frames"] = infos.get("num_seen_frames", 0)
    configs["step"] = infos.get("step", 0)
    ...
    if decoder is not None and model.decoder.tie_word_embedding:
