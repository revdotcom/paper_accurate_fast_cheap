...
from wenet.transformer.decoder import TransformerDecoder,  LanguageSpecificTransformerDecoder
...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        #print(f"TRANS speech inf or nan => {torch.isnan(speech).sum() + torch.isinf(speech).sum()}  of {speech.numel()}")
        #print(f"TRANS text   inf or nan => {torch.isnan(text).sum() + torch.isinf(text).sum()}  of {text.numel()}")
        ...
        #print(f"Encoder out Nan/Inf {torch.isnan(encoder_out).sum() + torch.isinf(encoder_out).sum()} on {encoder_out.numel()}")
