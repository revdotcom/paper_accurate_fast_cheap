...
class TeacherStudentASRModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.top_k_entries = top_k_entries
        self.float_top_k = float(self.top_k_entries)
        self.show_details = True
        if max_top_k_entries == -1:
            self.max_top_k_entries = self.top_k_entries 
        else:
            self.max_top_k_entries = max_top_k_entries 
    ...
    def forward(
        ...
        if self.show_details:
            print(f"We will do top k {self.top_k_entries} entries")

        if self.top_k_entries > 0:
            student_top_k_vals, student_top_k_inds = torch.topk(student_enc_ctc_outputs, self.top_k_entries, dim=2)
            teacher_top_k_vals, teacher_top_k_inds = torch.topk(teacher_enc_ctc_outputs, self.top_k_entries, dim=2)
        ...
        if self.top_k_entries > 0:
            ...
            student_top_k_vals, student_top_k_inds = torch.topk(student_dec_true_outputs, self.top_k_entries, dim=2)
            teacher_top_k_vals, teacher_top_k_inds = torch.topk(teacher_dec_true_outputs, self.top_k_entries, dim=2)
            ...
            if self.show_details:
                ...
                print(f"DECODER : we did top {self.top_k_entries} and got {kl_dec_loss} instead of {okl_dec_loss}") 
        else:
        ...
        if self.student.training and self.decrease_every > 0:
            ...
            if self.steps_forward >= self.decrease_every:
                ...
                self.float_top_k = (self.max_top_k_entries - self.float_top_k) * (1 - self.decrease_factor) + self.float_top_k
                self.top_k_entries = int(self.float_top_k)

        #print(f"mseloss / ctc loss = {mse_loss / loss_ctc}")
        return {"loss":ts_loss, 'ts_weight': self.ts_weight, 'kl_enc_loss':kl_enc_loss,'kl_dec_loss':kl_dec_loss, "student_loss":loss, "loss_att":loss_att, "loss_ctc": loss_ctc, "th_accuracy": acc_att, "top_k" : self.top_k_entries}
