...
class CTLModel(ASRModel):
    ...
    def forward(
        ...
        if 'cat_embs' in batch:
            # logging.info("Got real batch embeddings)")
            cat_embs = batch['cat_embs'].to(device)
        else:
            # logging.warning("cat_embs missing)")
            cat_embs = None

        loss_full, encoder_out_full, _, _, acc_att_full = self.forward_full(
            speech, speech_lengths, text, text_lengths, cat_embs=cat_embs)
        loss_chunk, encoder_out, lens_chunk, encoder_mask, acc_att_chunk = self.forward_chunk(
            speech, speech_lengths, text, text_lengths, cat_embs=cat_embs)
        ...
        return {
            ...
            "loss_ctl": ctl_loss,
            "th_accuracy": acc_att_full,
            "chunk_th_accuracy": acc_att_chunk
        }
    ...
    def forward_full(
        ...
        encoder_out, encoder_mask = self.encoder.forward_full(
            speech, speech_lengths, cat_embs=cat_embs)
        ...
        return loss, encoder_out, encoder_out_lens, encoder_mask, acc_att
