...
def detect_task(sample):
    ...
    #assert 'tar_file_name' in sample
    return sample
...
def tokenize(sample, tokenizer: BaseTokenizer, uppercase=False):
    ...
    assert 'txt' in sample
    if uppercase:
        tokens, label = tokenizer.tokenize(sample['txt'].upper())
    else:
...
def padding(data, pass_cat_emb=False, deep_biasing_conf={}):
    ...
    feats_lengths = torch.tensor([sample[i]['feat'].size(0) for i in order],
                                 dtype=torch.int32)
    #for i in order:
    #    print(f"   {i=} all finite {torch.isfinite(sample[i]['feat']).all()}")

    sorted_feats = [sample[i]['feat'] for i in order]
    sorted_keys = [sample[i]['key'] for i in order]
    shard_file_names = [sample[i].get('tar_file_name', 'no_shard') for i in order]
    sorted_labels = [
    ...
    padded_feats = pad_sequence(sorted_feats,
                                batch_first=True,
                                padding_value=0)
    #print(f"padded feats all finite : {torch.isfinite(padded_feats).all()}")
    ...
    batch = {
        ...
        "langs": langs,
        "tasks": tasks,
        "shard_file_names": shard_file_names,
    }
