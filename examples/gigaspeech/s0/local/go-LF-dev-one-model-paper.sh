...
function run_testset {
    ...
    result_dir=results.$pnick/dev/$nickname-b$batch_size.bs$beam_size.cs$xcs
    ...
    for f in /shared/speech-shards/dev/en/GigaSpeech_long/wav/*.wav; do
	x=$(basename $f .wav)
    ...
    python local/gigaspeech_scoring_longform.py '/shared/speech-shards/dev/en/GigaSpeech_long/*.ref_txt' "$result_dir/$decode_mode/*ctm" &
