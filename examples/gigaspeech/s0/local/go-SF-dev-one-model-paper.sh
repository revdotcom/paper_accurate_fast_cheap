...
for mode in $modes; do
   if [ ! -e $result_dir/$mode/results.wer ]; then
	python wenet/bin/recognize.py --gpu $3 $extras \
	--modes $modes \
	--config $config \
	--data_type "shard" \
	...
	python tools/compute-wer-giga.py --char=1 -v=1 /shared/speech-shards/dev/en/GigaSpeech/text $result_dir/$mode/text_nosw > $result_dir/$mode/results_nosw.filtered.wer
